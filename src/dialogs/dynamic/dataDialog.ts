...
export default class dataDialog extends dynamicDialogBase {
    ...
    private setupSteps() {
        ...
        if (this._dialog.data && this._dialog.data.fields && this._dialog.data.fields.length > 0) {
            for (let i in this._dialog.data.fields) {
                ...
                }else if(action && action.serviceUrlText){
                    step = this.textServiceStep(action.serviceUrlText);
                }
    ...
    initialStep() {
        return (session: builder.Session, args: any, next: Function) => {
            ...
            if (this._dialog.action && this._dialog.action.serviceUrlText) {
                var prompt: string = (this._dialog.action.textUrlPrompt || "");
    ...
    textServiceStep(textService: string) {
        return async (session: builder.Session, results: builder.IDialogResult<string>, next: Function) => {
            session.sendTyping();
            var action = this._dialog.action;
            var result = await this._netClient.postJson<any, contracts.serviceResult>(action.serviceUrlText, "", results);
            session.replaceDialog(this._dialog.id, {reprompt:true});
        }
    }
